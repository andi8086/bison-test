...
%token COS TAN ASIN ACOS ATAN INC DEC LAND OR COMPLEMENT
...
assign_expr: postfix_expr
        ...
        | REGA { int i;for(i=0;i<100;i++) if (reg[i]!=0)printf("%02d = %.2f\n",i,reg[i]); $$=0;}
        ;
...
set_format: function_call
        ...
        | SCI OPENBRACKET expr CLOSEBRACKET { sprintf(format,".%dg\n",(int)$3); $$=0; }
        | SCI { sprintf(format,"g\n"); $$=0; }
        | ENG OPENBRACKET expr CLOSEBRACKET { sprintf(format,".%de\n",(int)$3); $$=0; }
        | ENG { sprintf(format,"e\n"); $$=0; }
        ;
function_call: SIN OPENBRACKET expr CLOSEBRACKET { $$ = (cos($3)*tan($3)); }
